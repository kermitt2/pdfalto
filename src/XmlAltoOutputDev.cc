...
// inline bool operator==(const TextFontStyleInfo& lhs, const TextFontStyleInfo&
// rhs){
//     return (((lhs.getFontName())->cmp(rhs.getFontName())  == 0 ) &&
//     (lhs.getFontSize() == rhs.getFontSize())
...
/*ImageInline::ImageInline(double xPosition, double yPosition, double width,
                         double height, int idWord, int idImage, GString *href,
int index) { xPositionImage = xPosition; yPositionImage = yPosition; widthImage
= width; heightImage = height; idWordBefore = idWord; idImageCurrent = idImage;
    hrefImage = href;
    idx = index;
}
...
Image::Image(double xPosition, double yPosition, double width, double height,
             GString *id, GString *sid, GString *href, GString *clipzone,
             GBool isinline, double rotationA, GString *typeA) {
...
TextChar::TextChar(GfxState *stateA, Unicode cA, CharCode charCodeA,
                   int charPosA, int charLenA, double xMinA, double yMinA,
                   double xMaxA, double yMaxA, int rotA, GBool clippedA,
                   GBool invisibleA, TextFontInfo *fontA, double fontSizeA,
                   SplashFont *splashFontA, double colorRA, double colorGA,
                   double colorBA, GBool isNonUnicodeGlyphA) {
  double t;
  state = stateA;
  c = cA;
  charCode = charCodeA;
...
TextRawWord::TextRawWord(GfxState *state, double x0, double y0,
  ...
  if (state->getFont()) {
    if (state->getFont()->getName()) {
      // PDF reference guide 5.5.3 For a font subset, the PostScript name of the
      // font�the value of the font�s
      // BaseFont entry and the font descriptor�s FontName entry�begins with a
      // tag followed by a plus sign (+). The tag consists of exactly six
      ...
      // EOODIA+Poetica is the name of a subset of Poetica�, a Type 1 font. (See
      // implementation note 62 in Appendix H.)
      fontName = strdup(state->getFont()->getName()->getCString());
      char *localLowerFontName =
          state->getFont()->getName()->lowerCase()->getCString();
      if (strstr(localLowerFontName, "bold") ||
          strstr(localLowerFontName, "_bd"))
        bold = gTrue;
      if (strstr(localLowerFontName, "italic") ||
...
TextRawWord::~TextRawWord() {
  ...
  // Since chars are not owned by TextRawWord, we avoid deleting them brutally
  // with deleteGList. Instead, we cleanup the pointer to the list.
  // deleteGList(chars, TextChar);
  if (chars) {
...
void TextRawWord::setLineNumber(bool theBool) { lineNumber = theBool; }

Unicode TextRawWord::getChar(int idx) {
  return ((TextChar *)chars->get(idx))->c;
}

void TextRawWord::addChar(GfxState *state, double x, double y, double dx,
                          double dy, Unicode u, CharCode charCodeA,
                          int charPosA, GBool overlap, TextFontInfo *fontA,
                          double fontSizeA, SplashFont *splashFont, int rotA,
                          int nBytes, GBool isNonUnicodeGlyph) {
  ...
  if (overlap && len > 0) {
    ...
    if (leftClass != NOT_A_MODIFIER || rightClass != NOT_A_MODIFIER) {
      ...
      if (leftClass != NOT_A_MODIFIER) {
        ...
        // note that in this case we have to be careful with the word
        // coordinates, as the first character of the word might be a modifier,
        // we should use the base char instead otherwise we could introduce
        // spurious word break after the combined character due to shift from
        // the base line
      ...
      if (diactritic != 0) {
        ...
        if (!nfkc->isNormalized(*baseChar, errorCode)) {
          resultChar = nfkc->normalize(*baseChar, errorCode);
          resultChar = nfkc->normalizeSecondAndAppend(
              resultChar, *diacriticChar, errorCode);
        } else
          resultChar = nfkc->normalizeSecondAndAppend(*baseChar, *diacriticChar,
                                                      errorCode);
  ...
  if (!isUpdateAccentedChar) {
    ...
    charState->setCTM(state->getCTM()[0], state->getCTM()[1],
                      state->getCTM()[2], state->getCTM()[3],
                      state->getCTM()[4], state->getCTM()[5]);
    charState->setTextMat(state->getTextMat()[0], state->getTextMat()[1],
                          state->getTextMat()[2], state->getTextMat()[3],
                          state->getTextMat()[4], state->getTextMat()[5]);
    charState->setHorizScaling(state->getHorizScaling());

    chars->append(new TextChar(
        charState, u, charCodeA, charPosA, nBytes, xxMin, yyMin, xxMax, yyMax,
        rotA, clipped, state->getRender() == 3 || alpha < 0.001, fontA,
        fontSizeA, splashFont, colToDbl(rgb.r), colToDbl(rgb.g),
...
GBool TextRawWord::overlap(TextRawWord *w2) { return gFalse; }
...
TextLine::TextLine() { xMin = yMin = xMax = yMax = 0; }
...
TextParagraph::TextParagraph() { xMin = yMin = xMax = yMax = 0; }
...
TextParagraph::~TextParagraph() { deleteGList(lines, TextLine); }
...
void TextPage::setLineNumber(bool theBool) { lineNumber = theBool; }

/** get the presence of a column of line numbers in the text page */
bool TextPage::getLineNumber() { return lineNumber; }
    ...
    for (int i = 0; i < objAnnot.arrayGetLength(); ++i) {
      ...
      if (kid.isDict()) {
        ...
        if (dict->lookup("Subtype", &objSubtype)->isName()) {
          // It can be 'Highlight' or 'Underline' or 'Link' (Subtype 'Squiggly'
          // or 'StrikeOut' are not supported)
...
void TextPage::beginWord(GfxState *state, double x0, double y0) {
  ...
  idx++;
  curWord = new TextRawWord(state, x0, y0, curFont, curFontSize, getIdWORD(),
                            getIdx());
}
...
/*
 * Returns the correct base char for composition with icu4c following unicode
 * standard.
 */
...
void TextPage::addCharToPageChars(GfxState *state, double x, double y,
                                  double dx, double dy, CharCode c, int nBytes,
  ...
  if (uLen > 0) {
    ...
    for (i = 0; i < uLen; ++i) {
      ...
      //                    state->getClipBBox(&clipXMin, &clipYMin, &clipXMax,
      //                    &clipYMax); xMid = 0.5 * (xMin + xMax); yMid = 0.5 *
      //                    (yMin + yMax); if (xMid < clipXMin || xMid >
      //                    clipXMax ||
      ...
      chars->append(new TextChar(
          state->copy(), u[j], c, charPos, nBytes, xMin, yMin, xMax, yMax,
          curRot, clipped, state->getRender() == 3 || alpha < 0.001, curFont,
          curFontSize, splashFont, colToDbl(rgb.r), colToDbl(rgb.g),
          colToDbl(rgb.b), isNonUnicodeGlyph));
    }
  }
...
void TextPage::addCharToRawWord(GfxState *state, double x, double y, double dx,
                                double dy, CharCode c, int nBytes, Unicode *u,
                                int uLen, SplashFont *splashFont,
                                GBool isNonUnicodeGlyph) {
  ...
  // throw away chars that aren't inside the page bounds
  state->transform(x, y, &x1, &y1);
  if (x1 < 0 || x1 > pageWidth || y1 < 0 || y1 > pageHeight) {
    charPos += nBytes;
    endWord();
    return;
  }
  ...
  if (uLen == 1 && isUnicodeSpace(u[0])) {
    if (curWord) {
      ...
      if (curWord->chars->getLength() > 0)
        ((TextChar *)curWord->chars->get(curWord->chars->getLength() - 1))
            ->spaceAfter = (char)gTrue;
    }
    charPos += nBytes;
    endWord();
    return;
  ...
  if (curWord && curWord->len > 0) {
    ...
    if (curWord->len > 0)
      leftClass = classifyChar(
          ((TextChar *)curWord->chars->get(curWord->getLength() - 1))->c);
    ...
    if ((rightClass != NOT_A_MODIFIER || leftClass != NOT_A_MODIFIER)) {
      ...
      // left or right combination, it will consume the left or the right
      // character and possible break word will occur then after or before the
    ...
    if ((overlap || fabs(base - curWord->base) > 1 || space ||
    ...
    if (leftClass != NOT_A_MODIFIER && curWord && curWord->getLength() == 1) {
      // we keep track of the event composition for the next character, as the
      // base won't be reliable in the case the word starts with a composed
      // character
  ...
  if (uLen != 0) {
    ...
    if ((curWord->rot == 0 && w1 < 0) || (curWord->rot == 3 && h1 < 0) ||
        (curWord->rot == 2 && w1 > 0) || (curWord->rot == 1 && h1 > 0)) {
      endWord();
    ...
    for (i = 0; i < uLen; ++i) {
      ...
      curWord->addChar(
          state, x1 + i * w1, y1 + i * h1, w1, h1, u[i], c, charPos,
          (overlap || sp < -minDupBreakOverlap * curWord->fontSize), curFont,
          curFontSize, splashFont, nBytes, curRot, isNonUnicodeGlyph);
    }
  }
...
void TextPage::addChar(GfxState *state, double x, double y, double dx,
                       ...
                       SplashFont *splashFont, GBool isNonUnicodeGlyph) {
  //    if (parameters->getReadingOrder() == gTrue)
  //        addCharToPageChars(state, x, y, dx, dy, c, nBytes, u, uLen,
  //        splashFont, isNonUnicodeGlyph);
  //    else
  addCharToRawWord(state, x, y, dx, dy, c, nBytes, u, uLen, splashFont,
                   isNonUnicodeGlyph);
}
...
void TextPage::addAttributsNodeVerbose(xmlNodePtr node, char *tmp,
  ...
  xmlNewProp(node, (const xmlChar *)ATTR_SID,
             (const xmlChar *)buildSID(num, word->getIdx(), id)->getCString());
  delete id;
...
void TextPage::addAttributsNode(xmlNodePtr node, IWord *word,
                                TextFontStyleInfo *fontStyleInfo,
                                UnicodeMap *uMap, GBool fullFontName) {
  char tmp[10];
 ...
 */
void TextPage::testLinkedText(xmlNodePtr node, double xMin, double yMin,
        ...
        if (isOverlap) {
          ...
          case actionURI: {
            ...
            if (uri->isOk()) {
              ...
              if (dest != NULL) {
                // PL: the uri here must be well formed to avoid not wellformed
                // XML!
                xmlChar *name_uri =
                    xmlURIEscape((const xmlChar *)dest->getCString());
                // xmlNewProp(node, (const xmlChar*)ATTR_URILINK,(const
                // xmlChar*)dest->getCString());
          ...
          case actionGoTo: {
            ...
            if (goto_link->isOk()) {
              ...
              if (link_dest != NULL && link_dest->isOk()) {
                ...
                case destXYZ: {
                  ...
                  // if (link_dest->getChangeLeft() &&
                  // link_dest->getChangeTop()){
                  // TODO FH 25/01/2006 apply transform matrix of destination
                  // page, not current page
                  double x, y;
                  curstate->transform(link_dest->getLeft(), link_dest->getTop(),
                                      &x, &y);
                  snprintf(tmp, sizeof(tmp), "p-%d %g %g", page, x, y);
                  xmlNewProp(node, (const xmlChar *)ATTR_GOTOLINK,
                             (const xmlChar *)tmp);
                  //												printf("link
                  //%d %g %g\n",page,x,y);
                  return;
                  ...
                  // link to the page, without a specific location. PDF Data
                  // Destruction has hit again!
                case destFit:
                case destFitH:
                case destFitV:
                case destFitR:
            ...
            //							//sprintf()
            //							xmlNewProp(node,
            //(const xmlChar*)ATTR_LINKREF,(const xmlChar*)tmp);
          }
          case actionGoToR: {
            break;
          }
...
GBool TextPage::testOverlap(double x11, double y11, double x12, double y12,
                            double x21, double y21, double x22, double y22) {
  return ((min(x12, x22) >= max(x11, x21)) && (min(y12, y22) >= max(y11, y21)));
}

/**
 * Checks whether there are highlighted text.
 */
GBool TextPage::testAnnotatedText(double xMin, double yMin, double xMax,
                                  double yMax) {
  ...
  for (int i = 0; i < nb; i++) {
    ...
    if ((*dict).lookup("QuadPoints", &objQuadPoints)->isArray()) {
      ...
      if ((min(xMax, xmax2) >= max(xMin, xmin2)) &&
          (min(yMax, ymax2) >= max(yMin, ymin2))) {
        return gTrue;
      }
    }
    objQuadPoints.free();
...
GBool TextFontStyleInfo::cmp(TextFontStyleInfo *tsi) {
  if (((fontName && fontName->cmp(tsi->getFontName()) == 0) &&
       ...
       (fontType == tsi->getFontType()) && (fontWidth == tsi->getFontWidth()) &&
       (isbold == tsi->isBold()) && (isitalic == tsi->isItalic()) &&
...
TextBlock *TextPage::split(GList *charsA, int rot) {
  ...
  horizGaps = new GList();
  vertGaps = new GList();
  findGaps(charsA, rot, &xMin, &yMin, &xMax, &yMax, &avgFontSize, horizGaps,
           vertGaps);
  ...
  smallSplit = gFalse;
  if (rot & 1) {
    if (nHorizGaps > 0 && horizGapSize > vertGapSize &&
        horizGapSize > vertGapThreshold &&
        (minHorizChunkWidth > minChunk || nVertGaps == 0)) {
      doHorizSplit = gTrue;
    } else if (nVertGaps > 0) {
      doVertSplit = gTrue;
    ...
    if (nVertGaps > 0 && vertGapSize > horizGapSize &&
        vertGapSize > vertGapThreshold &&
        (minVertChunkWidth > minChunk || nHorizGaps == 0)) {
      doVertSplit = gTrue;
...
GList *TextPage::getChars(GList *charsA, double xMin, double yMin, double xMax,
                          double yMax) {
  GList *ret;
  TextChar *ch;
  double x, y;
...
void TextPage::insertLargeChars(GList *largeChars, TextBlock *blk) {
  ...
  for (i = 1; i < largeChars->getLength(); ++i) {
    ...
    minOverlap =
        0.5 * (ch->fontSize < ch2->fontSize ? ch->fontSize : ch2->fontSize);
...
void TextPage::findGaps(GList *charsA, int rot, double *xMinOut,
                        double *yMinOut, double *xMaxOut, double *yMaxOut,
                        double *avgFontSizeOut, GList *horizGaps,
                        GList *vertGaps) {
  TextChar *ch;
  ...
  for (y = start; y < yMaxI; ++y) {
    if (horizProfile[y - yMinI]) {
      ...
      if (horizProfile[y + 1 - yMinI]) {
        ...
        horizGaps->append(
            new TextGap((start + 1) * splitPrecision + 0.5 * w, w));
      }
    }
  }
...
double TextPage::computeWordSpacingThreshold(GList *charsA, int rot) {
  ...
  if (maxGap - minGap < uniformSpacing * avgFontSize) {
    if (minAdjGap <= maxAdjGap && minSpGap <= maxSpGap &&
...
//                    }
    ...
    for (int i = 0; i < imageCount; ++i) {
        ...
        snprintf(tmp, 10, ATTR_NUMFORMAT, listeImages[i]->getXPositionImage());
        xmlNewProp(node, (const xmlChar *) ATTR_X, (const xmlChar *) tmp);
        snprintf(tmp, 10, ATTR_NUMFORMAT, listeImages[i]->getYPositionImage());
        xmlNewProp(node, (const xmlChar *) ATTR_Y, (const xmlChar *) tmp);
        snprintf(tmp, 10, ATTR_NUMFORMAT, listeImages[i]->getWidthImage());
        xmlNewProp(node, (const xmlChar *) ATTR_WIDTH, (const xmlChar *) tmp);
        snprintf(tmp, 10, ATTR_NUMFORMAT, listeImages[i]->getHeightImage());
        xmlNewProp(node, (const xmlChar *) ATTR_HEIGHT, (const xmlChar *) tmp);
...
bool is_digit(Unicode u) {
  // simply match Unicode for ASCII digit... should we add more unicode variants
  // of numbers?
  if (u == (Unicode)48 || u == (Unicode)49 || u == (Unicode)50 ||
      u == (Unicode)51 || u == (Unicode)52 || u == (Unicode)53 ||
      u == (Unicode)54 || u == (Unicode)55 || u == (Unicode)56 ||
      u == (Unicode)57)
    return true;
  else
    return false;
...
bool TextPage::markLineNumber() {
  // Detect the presence of line number column in the page and mark the
  // corresponding TextWord objects for further appropriate handling
  ...
  // - immediate vertical gap with next token vertically aligned (note that this
  // is actually complicated with the PDF stream order)
  ...
  // first pass is for number detection, it should stop very early in case of no
  // line number we also keep track of the left and right text boundaries for
  // the page
  ...
  // select largest cluster of numbers, which will give the best x alignments
  // and the corresponding lists of number tokens
    ...
    if (bestClusterIndex.size() == 0) {
      ...
      while (it1 != last) {
        if (theCluster.size() < largestClusterSize[j]) {
          ...
          // this is sorted based on the cluster siez, so we insert just before
          // j
  ...
  */
  ...
  // Do we have text areas at same alignment or positioned more on the side than
  // the number cluster?
  // -> see the left-most and right-most non trivial text block with the text
  // token clusters

  // cout << "totalNumberOfLines: " << totalNumberOfLines << endl;

  // first the text area must be large enough, this depends on the overall
  ...
  for (int j = 0; j < bestClusterIndex.size(); j++) {
    ...
    for (int i = 0; i < textClusters.size(); i++) {
      ...
      if (theCluster.size() >= nonTrivialClusterSize) {
        ...
        // check top and lowest position of the text cluster, if too high or too
        // low, it means it's head note or foot note and should not be
        // considered
        int ypos1 = word->yMin;
        ...
        if ((vpos1 <= final_vpos) && (vpos2 >= final_vpos)) {
          hasLineNumber = false;
          // cout << "           breaking text cluster has a size of: " <<
          // theCluster.size() << endl;
          break;
        }
      }
    }
  ...
  // (these are ref numbers in the biblio or something else, but can't be line
  // numbers)
  int quarterWidth = (rightMostBoundary - leftMostBoundary) / 4;
  if (quarterWidth + leftMostBoundary < final_vpos &&
      final_vpos < leftMostBoundary + (quarterWidth * 3))
    hasLineNumber = false;
  ...
  // increment? it's not possible to suppose any particular increments, it could
  // be 1 by 1 or 5 by 5 for instance, however number should be growing! to be
  // done if needed...
...
void TextPage::dump(GBool noLineNumbers, GBool fullFontName,
                    vector<bool> lineNumberStatus) {
  // Output the page in raw (content stream) order
  blocks = new GList(); // these are blocks in alto schema
  vector<TextParagraph *>
      originalBlocks; // only used when reading order is selected
  ...
  GBool lastBlockInserted =
      gFalse; // true if the last added block is inserted before an existing
              // node, false is appended
  ...
  for (wordId = 0; wordId < words->getLength(); wordId++) {
    ...
    //		- and IF the rotation of current word is different of the
    // rotation next word
    //		- and IF the difference between the base of current word and the
    // yMin next word is superior to the maxSpacingWordsBetweenTwoLines
    //		-      or IF the difference between the base of current word and
    // the base next word is superior to maxIntraLineDelta * lineFontSize
    //		- and IF the xMax current word ++ maxWordSpacing * lineFontSize
    // is superior to the xMin next word. 		HD 24/07/09 ?? - or if
    // the font size of the current word is far different from the next word
    // PL: note that the second criteria create an artificial new line for every
    // superscript sequences whithin a line (but subscript are okay), we revised
    // it by taking the max of lineFontSize and nextWord fontSize, instead of
    // the min (it means we have a new line when the shift if greater than the
    // main font size, a smaller shift does not introduce a new line)
    if (nextWord != NULL && (word->rot == nextWord->rot) &&
        (((fabs(word->base - nextWord->baseYmin) <
           maxSpacingWordsBetweenTwoLines) ||
          //(fabs(nextWord->base - word->base) < maxIntraLineDelta *
          // min(lineFontSize, nextWord->fontSize)) (fabs(nextWord->base -
          // word->base) < maxIntraLineDelta * lineFontSize)
          (fabs(nextWord->base - word->base) <
           maxIntraLineDelta * max(lineFontSize, nextWord->fontSize))) &&
         (nextWord->xMin <= word->xMax + maxWordSpacing * lineFontSize))) {

      // IF - switch the rotation :
      //			base word and yMin word are inferior to yMin
      // next word 			xMin word is superior to xMin next word
      // xMax word is superior to xMin next word and the difference between the
      // base of current word and the next word is superior to
      // maxIntraLineDelta*lineFontSize 			xMin next word
      // is superior to xMax word + maxWordSpacing * lineFontSize THEN
      // if one of these tests is true, the line is finish
      if (((rotation == -1) ? ((word->base < nextWord->yMin) &&
                               (word->yMin < nextWord->yMin))
           : (word->rot == 0 || word->rot == 1)
               ? ((word->base < yyMinNext) && (yyMin < yyMinNext))
               : ((word->base > yyMinNext) && (yyMin > yyMinNext))) ||
          ((rotation == -1) ? (nextWord->xMin < word->xMin)
           : (word->rot == 0)
               ? (xxMinNext < xxMin)
               : (word->rot == 1 ? xxMinNext > xxMin
                                 : (word->rot == 2 ? xxMinNext > xxMin
                                                   : xxMinNext < xxMin))) ||
          ((rotation == -1) ? (nextWord->xMin < word->xMax) &&
                                  (fabs(nextWord->base - word->base) >
                                   maxIntraLineDelta * lineFontSize)
           : (word->rot == 0 || word->rot == 3)
               ? ((xxMinNext < xxMax) && (fabs(nextWord->base - word->base) >
                                          maxIntraLineDelta * lineFontSize))
               : ((xxMinNext > xxMax) && (fabs(nextWord->base - word->base) >
                                          maxIntraLineDelta * lineFontSize))) ||
          ((rotation == -1)
               ? (nextWord->xMin > word->xMax + maxWordSpacing * lineFontSize)
           : (word->rot == 0 || word->rot == 3)
               ? (xxMinNext > xxMax + maxWordSpacing * lineFontSize)
               : (xxMinNext < xxMax - maxWordSpacing * lineFontSize))) {
        lineWords->append(word); // here we first add this last word to line
    ...
    if ((lineFinish) || (endPage)) {
      ...
      if (linePreviousX == 0) {
        if (nextWord != NULL) {
          if (nextWord->xMin >
              (lineX + lineWidth) + (maxColSpacing * lineFontSize)) {
            newBlock = gTrue;
          }
        }
        ...
        if (newBlock) {
          ...
          if (paragraph != NULL) {
            ...
            // double blockWidth = std::abs((linePreviousX + linePreviousWidth)
            // - blockX);
            double blockHeight =
                std::abs((linePreviousYmin + linePreviousHeight) - blockY);
          ...
          if (((lineYmin + lineHeight) >= linePreviousYmin) &&
              ...
              ((lineYmin - linePreviousYmin) <
               (linePreviousFontSize * maxLineSpacingDelta))) {
            if (endPage) {
              // PL: check if the width and the height of the current block are
              // present and set them if it's not the case
              if (paragraph != NULL) {
                if (paragraph->getYMax() == 0) {
                  double blockY = paragraph->getYMin();
                  double blockHeight = std::abs(
                      (linePreviousYmin + linePreviousHeight) - blockY);
                  paragraph->setYMax(blockY + blockHeight);
                }
                if (paragraph->getXMax() == 0) {
                  double blockX = paragraph->getXMin();
                  // double blockWidth = std::abs((linePreviousX +
                  // linePreviousWidth) - blockX);
            ...
            if (paragraph != NULL) {
              ...
              // double blockWidth = std::abs((linePreviousX +
              // linePreviousWidth) - paragraph->getXMin());
              double blockHeight =
                  std::abs((linePreviousYmin + linePreviousHeight) -
                           paragraph->getYMin());
  ...
  // fix possible incorrect XMax and YMax values at 0 on block coordinates
  ...
  // if no line number was found in the first half of the document and the
  // number of pages of the document is large enough, we do don't need to look
  // for line numbers any more
  ...
  bool hasLineNumber = false;
  if ((currentPageNumber < nbTotalPage / 2) ||
      (previousLineNumber && nbTotalPage > 4)) {
  ...
  if (hasLineNumber && !noLineNumbers) {
    // we preliminary introduce a block for line numbers, if any and if we have
    // to display them
    ...
    if (lineNumberWords->getLength() > 0) {
      ...
      id = new GString("p");
      xmlNewProp(
          nodeblocks, (const xmlChar *)ATTR_ID,
          (const xmlChar *)buildIdBlock(num, numBlock, id)->getCString());
      delete id;
      numBlock = numBlock + 1;
      ...
      xmlNewProp(nodeblocks, (const xmlChar *)ATTR_HEIGHT,
                 (const xmlChar *)tmp);
      ...
      for (wordI = 0; wordI < lineNumberWords->getLength(); wordI++) {
        ...
        id = new GString("p");
        xmlNewProp(
            nodeline, (const xmlChar *)ATTR_ID,
  ...
  for (parIdx = 0; parIdx < blocks->getLength(); parIdx++) {
    ...
    for (lineIdx = 0; lineIdx < par->lines->getLength(); lineIdx++) {
      ...
      snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
               line1->getXMax() - line1->getXMin());
      xmlNewProp(nodeline, (const xmlChar *)ATTR_WIDTH, (const xmlChar *)tmp);
      snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
               line1->getYMax() - line1->getYMin());
      ...
      for (wordI = 0; wordI < line1->words->getLength(); wordI++) {
        ...
        // determine if the current token is superscript of subscript: a general
        // constraint for superscript and subscript is that the scripted tokens
        // have a font size smaller than the tokens on the line baseline.

        // superscript
        if (currentLineBaseLine != 0 && wordI > 0 &&
            word->base < currentLineBaseLine && word->yMax > currentLineYmin &&
            word->fontSize < lineFontSize) {
          // superscript: general case, not at the beginning of a line
          fontStyleInfo->setIsSuperscript(gTrue);
        } else if (wordI == 0 && wordI < line1->words->getLength() - 1 &&
                   nextWord != NULL && word->base < nextWord->base &&
                   word->yMax > nextWord->yMin &&
                   word->fontSize < lineFontSize) {
          // superscript: case first token of the line, check if the current
          // token is the first token of the line and use next tokens to see if
          // we have a vertical shift note: it won't work when we have several
          // tokens in superscript at the beginning of the line... actually it
          // might screw all the rest :/ superscript as first token of a line is
          // common for declaring affiliations (and sometime references)
          fontStyleInfo->setIsSuperscript(gTrue);
          currentLineBaseLine = nextWord->base;
          currentLineYmin = nextWord->yMin;
          currentLineYmax = nextWord->yMax;
        } else if (wordI > 0 && word->base > currentLineBaseLine &&
                   word->yMin < currentLineYmax &&
                   word->fontSize < lineFontSize) {
          // common subscript, not at the beginning of a line
          fontStyleInfo->setIsSubscript(gTrue);
                   ...
                   word->yMin < nextWord->yMax &&
                   word->fontSize < lineFontSize) {
          // subscript: case first token of the line, check if the current token
          // is the first token of the line and use next tokens to see if we
          // have a vertical shift note: it won't work when we have several
          // tokens in subscripts at the beginning of the line... actually it
          // might screw all the rest :/ subscript as first token of a line
          // should never appear, but it's better to cover this case to avoid
          // having the rest of the line detected as superscript...
          fontStyleInfo->setIsSubscript(gTrue);
          currentLineBaseLine = nextWord->base;
          currentLineYmin = nextWord->yMin;
          currentLineYmax = nextWord->yMax;
        }
        // PL: above, we need to pay attention to the font style of the previous
        // token and consider the whole line, because otherwise the token next
        // to a subscript is always superscript even when normal, in addition
        // for several tokens as superscript or subscript, only the first one
        // will be set as superscript or subscript
        ...
        //                    encodeFragment(line->text, n, uMap, primaryLR, s);
        //                    if (lineIdx + 1 < par->lines->getLength() &&
        //                    !line->hyphenated) {
        //                        s->append(space, spaceLen);
        //                    }

        // Add next images inline whithin the current line if the noImageInline
        ...
        /*if (parameters->getImageInline()) {
            if (indiceImage != -1) {
                ...
                for (; indiceImage < nb; indiceImage++) {
                    if (idWORDBefore ==
        listeImageInline[indiceImage]->idWordBefore) { nodeImageInline =
        xmlNewNode(NULL, (const xmlChar *) TAG_TOKEN); nodeImageInline->type =
        XML_ELEMENT_NODE; id = new GString("p"); xmlNewProp(nodeImageInline,
        (const xmlChar *) ATTR_ID, (const xmlChar *) buildIdToken(num, numToken,
        id)->getCString()); delete id; numToken = numToken + 1; id = new
        GString("p"); xmlNewProp( nodeImageInline, (const xmlChar *) ATTR_SID,
                                (const xmlChar *) buildSID(num,
        listeImageInline[indiceImage]->getIdx(), id)->getCString()); delete id;
                        snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
                                listeImageInline[indiceImage]->getXPositionImage());
                        xmlNewProp(nodeImageInline, (const xmlChar *) ATTR_X,
                                   (const xmlChar *) tmp);
                        ...
                        snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
                                 listeImageInline[indiceImage]->getWidthImage());
                        xmlNewProp(nodeImageInline, (const xmlChar *)
        ATTR_WIDTH, (const xmlChar *) tmp); snprintf(tmp, sizeof(tmp),
        ATTR_NUMFORMAT, listeImageInline[indiceImage]->getHeightImage());
                        xmlNewProp(nodeImageInline,
                                   (const xmlChar *) ATTR_HEIGHT,
                                   (const xmlChar *) tmp);
                        xmlNewProp(
                                ...
                                (const xmlChar *) ATTR_HREF,
                                (const xmlChar *)
        listeImageInline[indiceImage]->getHrefImage()->getCString());
                        xmlAddChild(nodeline, nodeImageInline);
                    }
                }
            }
        ...
        //			    if
        //(testAnnotatedText(minLineX,minLineY,minLineX+lineWidth,minLineY+lineHeight)){
        //				    xmlNewProp(nodeline, (const
        // xmlChar*)ATTR_HIGHLIGHT,(const xmlChar*)"yes");
        //			    }
        ...
        if (!word->getLineNumber()) {
          ...
          if (wordI < line1->words->getLength() - 1 and
              (word->spaceAfter == gTrue)) {
            xmlNodePtr spacingNode =
                xmlNewNode(NULL, (const xmlChar *)TAG_SPACING);
            spacingNode->type = XML_ELEMENT_NODE;
            snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
  ...
  for (int i = 0; i < imageCount; ++i) {
    ...
    node = xmlNewNode(NULL, (const xmlChar *)TAG_IMAGE);
    xmlNewProp(node, (const xmlChar *)ATTR_ID,
               (const xmlChar *)listeImages[i]->getImageId()->getCString());

    // xmlNewProp(node, (const xmlChar *) ATTR_SID,(const
    // xmlChar*)listeImages[i]->getImageSid()->getCString());

    snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
             listeImages[i]->getXPositionImage());
    xmlNewProp(node, (const xmlChar *)ATTR_X, (const xmlChar *)tmp);
    snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
             listeImages[i]->getYPositionImage());
    xmlNewProp(node, (const xmlChar *)ATTR_Y, (const xmlChar *)tmp);
    snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT, listeImages[i]->getWidthImage());
    xmlNewProp(node, (const xmlChar *)ATTR_WIDTH, (const xmlChar *)tmp);
    snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
             listeImages[i]->getHeightImage());
    xmlNewProp(node, (const xmlChar *)ATTR_HEIGHT, (const xmlChar *)tmp);

    std::string rotation = std::to_string(listeImages[i]->getRotation());
    xmlNewProp(node, (const xmlChar *)ATTR_ROTATION,
               (const xmlChar *)rotation.c_str());
    // if (listeImages[i]->getRotation() > 0){
    //     xmlNewProp(node,(const xmlChar*)ATTR_ROTATION,(const xmlChar*)sTRUE);
    // }
    // else{
    //     xmlNewProp(node,(const xmlChar*)ATTR_ROTATION,(const
    //     xmlChar*)sFALSE);
    // }

    //        if (listeImages[i]->isImageInline()) {
    //            xmlNewProp(node, (const xmlChar *) ATTR_INLINE, (const xmlChar
    //            *) sTRUE);
    //        }
    ...
    /*if (verbose) {
        xmlNewProp(node, (const xmlChar *) ATTR_CLIPZONE,
                   (const xmlChar *)
    listeImages[i]->getClipZone()->getCString());
    }*/
    xmlAddChild(printSpace, node);
  }
  ...
  // if (parameters->getDisplayImage())
  {
    ...
    xmlNewProp(node, (const xmlChar *)ATTR_ROTATION,
               (const xmlChar *)rotation.c_str());
    // if (r > 0) {
    //     xmlNewProp(node, (const xmlChar *) ATTR_ROTATION, (const xmlChar *)
    //     sTRUE);
    // } else {
    //     xmlNewProp(node, (const xmlChar *) ATTR_ROTATION, (const xmlChar *)
    //     sFALSE);
...
void TextPage::addBlockInTopDownOrder(TextParagraph *block) {
  ...
  if (nbChildren > 0) {
    ...
    // we get all the block nodes corresponding to the page, starting from the
    // lower/last present block to prioritize the stream order when it is valid
...
void TextPage::blocksInReadingOrder(vector<TextParagraph *> originalBlocks) {
  ...
  for (int j = 0; j < blocks->getLength(); j++) {
    ...
    for (int i = 0; i < blocks->getLength(); i++) {
      ...
      // we introduce a margin error to avoid too much fragmentation due to
      // paragraph/section header separation in columns, we consider +- the
      // height of a line
      ...
      if (areaSimple.at(j) == true) {
        ...
        if (newBlocks.size() == 0) {
          ...
          for (int i = localBlocks.size() - 1; i >= 0; i--) {
            ...
            double c_w = currentBlock->getXMax() - currentBlock->getXMin();
            double c_h = currentBlock->getYMax() - currentBlock->getYMin();
            // cout << "comparing " << block << " with " << currentBlock <<
            // endl; cout << "to be inserted: " << block << "(" <<
            // block->lines->getLength() << " lines)" << " X: " << x << ", Y: "
            // << y << ", H: " << h << ", W: " << w << ", X_max: " <<
            // block->getXMax() << ", Y_max: " << block->getYMax() << endl; cout
            // << "current: " << currentBlock << "(" <<
            // currentBlock->lines->getLength() << " lines)" << " X: " << c_x <<
            // ", Y: " << c_y << ", H: " << c_h << ", W: " << c_w << endl;

            if (y == c_y) {
              if (x + w < c_x) {
                // although same level, block to be added is entirely on the
                // left of current, so before in reading order
                insertIndex = i;
              } else if ((x > c_x + c_w)) {
                // although same level, the block to be inserted is on the right
                // of the current block block is after current, we look at lower
                // block(s)
                break;
              }
            } else if (y > c_y) {
              if (x + w < c_x) {
                // although lower, block to be added is entirely on the left of
                // current, so before in reading order
                insertIndex = i;
                // cout << "block on the left: " << block << " when compared to
                // " << currentBlock << endl;
              } else {
              ...
              if ((x > c_x + c_w)) {
                // although higher the block to be inserted is on the right of
                // the current block block is after current, we look at lower
                // block(s)
                break;
              } else
                insertIndex = i;
          ...
          for(int i=0; i<localBlocks.size(); i++) {
              cout << localBlocks.at(i) << "(" <<
          localBlocks.at(i)->lines->getLength() << " lines)" << endl;
          }*/
        }
      }
    }
...
// PL: sort blocks on a page in reading order
// this version reorder areas, but keep PDF stream order for the blocks located
// in the areas
void TextPage::blocksInReadingOrderVariant(
    vector<TextParagraph *> originalBlocks) {
  // cout << blocks->getLength() << " elements " << endl;
  //  blocks are already sorted top down
  vector<int> areaIndex;
  vector<bool> areaSimple;
    ...
    for (int i = 0; i < originalBlocks.size(); i++) {
      ...
      for (int j = 0; j < blocks->getLength(); j++) {
        ...
        if (originalBlocks.at(i) == block) {
          // not sure that pointer/reference comparison for instance
          // identification is safe, but it should
...
/**
 * Use to detect majors issues in reading order for the blocks in the current
 * page.
 */
...
/*void TextPage::addImageInlineNode(xmlNodePtr nodeline,
                                  xmlNodePtr nodeImageInline, char *tmp, IWord
*word) { indiceImage = -1; idWORDBefore = -1; GBool first = gTrue; int nb =
listeImageInline.size(); for (int i = 0; i < nb; i++) { if (word->idWord ==
listeImageInline[i]->idWordBefore) { if
(listeImageInline[i]->getXPositionImage() > word->xMax
                ...
                id = new GString("p");
                xmlNewProp(nodeImageInline, (const xmlChar *) ATTR_ID,
                           (const xmlChar *) buildIdToken(num, numToken,
id)->getCString()); delete id; numToken = numToken + 1; id = new GString("p");
                xmlNewProp(nodeImageInline, (const xmlChar *) ATTR_SID,
                           (const xmlChar *) buildSID(num,
listeImageInline[i]->getIdx(), id)->getCString()); delete id; snprintf(tmp,
sizeof(tmp), ATTR_NUMFORMAT, listeImageInline[i]->getXPositionImage());
                xmlNewProp(nodeImageInline, (const xmlChar *) ATTR_X,
                           (const xmlChar *) tmp);
                snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
                ...
                xmlNewProp(nodeImageInline, (const xmlChar *) ATTR_HREF,
                           (const xmlChar *)
listeImageInline[i]->getHrefImage()->getCString()); xmlAddChild(nodeline,
nodeImageInline); idImageInline = listeImageInline[i]->getIdImageCurrent();
            }
            int j = i;
            for (; j < nb; j++) {
                if (word->idWord == listeImageInline[j]->idWordBefore) {
                    if (listeImageInline[j]->getXPositionImage() > word->xMax
                        ...
                        if (idImageInline
                            ...
                            nodeImageInline = xmlNewNode(NULL,
                                                         (const xmlChar *)
TAG_TOKEN); nodeImageInline->type = XML_ELEMENT_NODE; GString *id; id = new
GString("p"); xmlNewProp(nodeImageInline, (const xmlChar *) ATTR_ID, (const
xmlChar *) buildIdToken(num, numToken, id)->getCString()); delete id; numToken =
numToken + 1; id = new GString("p"); xmlNewProp(nodeImageInline, (const xmlChar
*) ATTR_SID, (const xmlChar *) buildSID(num, listeImageInline[j]->getIdx(),
                                                                  id)->getCString());
                            delete id;
                            snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
                                     listeImageInline[j]->getXPositionImage());
                            xmlNewProp(nodeImageInline, (const xmlChar *)
ATTR_X, (const xmlChar *) tmp); snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
                                     listeImageInline[j]->getYPositionImage());
                            xmlNewProp(nodeImageInline, (const xmlChar *)
ATTR_Y, (const xmlChar *) tmp); snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT,
                            ...
                            xmlNewProp(
                                    ...
                                    (const xmlChar *) ATTR_HREF,
                                    (const xmlChar *)
listeImageInline[j]->getHrefImage()->getCString()); xmlAddChild(nodeline,
nodeImageInline); idImageInline = listeImageInline[j]->getIdImageCurrent();
                        }
                    } else {
                        if (first) {
                            indiceImage = j;
...
void TextPage::saveState(GfxState *state) { idStack.push(idCur); }
...
void TextPage::doPath(GfxPath *path, GfxState *state, GString *gattributes) {
  ...
  // if (parameters->getDisplayImage())
  {
    ...
    xmlNewProp(groupNode, (const xmlChar *)ATTR_SVGID,
               (const xmlChar *)sid.getCString());
    // xmlNewProp(groupNode, (const xmlChar *) ATTR_IDCLIPZONE, (const xmlChar
    // *) clipZone->getCString());
    createPath(path, state, groupNode);
  }
}

void TextPage::createPath(GfxPath *path, GfxState *state,
  ...
  for (i = 0; i < n; ++i) {
    ...
    while (j < m) {
      if (subpath->getCurve(j)) {
        ...
        snprintf(tmp, SVG_VALUE_BUFFER_SIZE,
                 " C%1.4f,%1.4f %1.4f,%1.4f %1.4f,%1.4f", x1, y1, x2, y2, x3,
                 y3);
        dd.append(tmp);
        if (xmax == 0) {
    ...
    xmlNewProp(pathnode, (const xmlChar *)ATTR_D,
               (const xmlChar *)dd.getCString());
    xmlAddChild(groupNode, pathnode);
  }
...
void TextPage::clip(GfxState *state) {
  ...
  // if (parameters->getDisplayImage())
  {
    ...
    xmlNewProp(gnode, (const xmlChar *)ATTR_SVGID,
               (const xmlChar *)sid.getCString());
    // xmlNewProp(gnode, (const xmlChar *) ATTR_IDCLIPZONE, (const xmlChar *)
    // clipZone->getCString());
    //    	free(tmp);
...
void TextPage::eoClip(GfxState *state) {
  ...
  // if (parameters->getDisplayImage())
  {
    ...
    GString *id = new GString("p"),
            *sid = new GString("p"); //, *clipZone = new GString("p");
    GBool isInline = false;
    id = buildIdImage(getPageNumber(), numImage, id);
    sid = buildSID(getPageNumber(), getIdx(), sid);
    // clipZone = buildIdClipZone(getPageNumber(), idCur, clipZone);
...
//// Draw the image mask
// const char* TextPage::drawImageMask(GfxState *state, Object *ref, Stream
// *str,
//                              int width, int height, GBool invert, GBool
//                              inlineImg, GBool dumpJPEG, int imageIndex) {
//
//
...
//             //error(-1, "Couldn't open image file '%s'",
//             relname->getCString()); return extension;
//         }
//         fprintf(f, "P4\n");
//         fprintf(f, "%d %d\n", width, height);
//
...
//         GString *id;
//         id = new GString("p");
//         xmlNewProp(node, (const xmlChar*)ATTR_ID, (const
//         xmlChar*)buildIdImage(num, numImage, id)->getCString()); delete id;
//         numImage = numImage + 1;
//
//         id = new GString("p");
//         xmlNewProp(node, (const xmlChar*)ATTR_SID, (const
//         xmlChar*)buildSID(num, getIdx(), id)->getCString()); delete id;
//
...
//         xmlNewProp(node, (const xmlChar*)ATTR_HEIGHT, (const xmlChar*)tmp);
//         if (inlineImg) {
//             xmlNewProp(node, (const xmlChar*)ATTR_INLINE, (const
//             xmlChar*)sTRUE);
//         }
...
//         listeImageInline.push_back(new ImageInline(x0, y0, w0, h0,
//         getIdWORD(), imageIndex, refname, getIdx()));
//     }
//
//     id = new GString("p");
//     xmlNewProp(node, (const xmlChar*)ATTR_CLIPZONE,
//                (const xmlChar*)buildIdClipZone(num, idCur,
//                id)->getCString());
//     delete id;
//     //  free(tmp);
//     return extension;
...
const char *TextPage::drawImageOrMask(GfxState *state, Object *ref, Stream *str,
                                      ...
                                      int * /* maskColors */, GBool extractImg,
                                      GBool mask, int imageIndex) {
  GString pic_file;
      ...
      if (data != NULL) {
        ...
        if (flip_x) {
          ...
          for (k = 0; k < total; k++) {
            a = data[k];
            a = (a >> 4) + (a << 4);
            a = ((a & 0xCC /* 11001100b */) >> 2) +
                ((a & 0x33 /* 00110011b */) << 2);
            data[k] = ((a & 0xAA /* 10101010b */) >> 1) +
                      ((a & 0x55 /* 01010101b */) << 1);
          }
          ...
          if (complementary_shift != 0) {
            ...
            unsigned char mask = 0xFF
                                 << complementary_shift; // mask for remainder
            unsigned char b;
            unsigned char remainder =
                0; // remainder is part that comes out when shifting
        ...
        // Save PNG file
        save_png(relname, width, height, stride, data, 1,
                 PNG_COLOR_TYPE_PALETTE, palette, 2);
    ...
    // ------------------------------------------------------------
      ...
      if (data != NULL) {
        if (colorMap->getBits() > 0) {
          ImageStream *imgStr = new ImageStream(
          ...
          // Save PNG file
          save_png(relname, width, height, width * 3, data, 24,
                   PNG_COLOR_TYPE_RGB, NULL, 0);
  ...
  // if (!inlineImg)
  {
    GString *id = new GString("p"), *sid = new GString("p"),
            *clipZone = new GString("p");
    GBool isInline = false;
    id = buildIdImage(getPageNumber(), numImage, id);
    sid = buildSID(getPageNumber(), getIdx(), sid);
    ...
    listeImages.push_back(new Image(x0, y0, w0, h0, id, sid, refname, clipZone,
                                    isInline, 0, new GString("image")));
    //        delete sid;
    //        delete id;
    //        delete clipZone;
  }
  ...
  /*if (inlineImg && parameters->getImageInline()) {
      listeImageInline.push_back(new ImageInline(x0, y0, w0, h0, getIdWORD(),
  imageIndex, refname, getIdx()));
  }*/

  return extension;
  //	append_image_block(round(x1), round(y1), round(x2-x1), round(y2-y1),
  // pic_file);
}
...
bool TextPage::save_png(GString *file_name, unsigned int width,
                        unsigned int height, unsigned int row_stride,
                        unsigned char *data, unsigned char bpp,
                        unsigned char color_type, png_color *palette,
  ...
  // Writing functions
  png_set_write_fn(png_ptr, file, (png_rw_ptr)file_write_data,
                   (png_flush_ptr)file_flush_data);
  ...
  // Metrics
  png_set_pHYs(png_ptr, info_ptr, 3780, 3780,
               PNG_RESOLUTION_METER); // 3780 dot per meter

  // Palette
  if (palette != NULL) {
    png_set_IHDR(png_ptr, info_ptr, info_ptr->width, info_ptr->height,
                 info_ptr->bit_depth, PNG_COLOR_TYPE_PALETTE,
                 info_ptr->interlace_type, PNG_COMPRESSION_TYPE_BASE,
...
XmlAltoOutputDev::XmlAltoOutputDev(GString *fileName, GString *fileNamePdf,
                                   Catalog *catalog, GBool physLayoutA,
                                   GBool verboseA, GString *nsURIA,
                                   GString *cmdA) {
  text = NULL;
  splash = NULL;
  fontEngine = NULL;
  ...
  // initialise some special unicodes 9 to begin with as placeholders, from
  // https://unicode.org/charts/PDF/U2B00.pdf
  ...
  xmlNsPtr nsXi = xmlNewNs(docroot, (const xmlChar *)ALTO_URI, NULL);
  ...
  // here we add basic structure : see
  // https://www.loc.gov/standards/alto/techcenter/structure.html
  xmlNodePtr nodeDescription =
      xmlNewNode(NULL, (const xmlChar *)TAG_DESCRIPTION);
  ...
  xmlNodePtr nodeMeasurementUnit =
      xmlNewNode(NULL, (const xmlChar *)TAG_MEASUREMENTUNIT);
  nodeMeasurementUnit->type = XML_ELEMENT_NODE;
  xmlNodeSetContent(
      nodeMeasurementUnit,
      (const xmlChar *)xmlEncodeEntitiesReentrant(
          nodeMeasurementUnit->doc, (const xmlChar *)MEASUREMENT_UNIT));
  xmlAddChild(nodeDescription, nodeMeasurementUnit);

  xmlNodePtr nodeSourceImageInformation =
      xmlNewNode(NULL, (const xmlChar *)TAG_SOURCE_IMAGE_INFO);
  nodeSourceImageInformation->type = XML_ELEMENT_NODE;
  xmlAddChild(nodeDescription, nodeSourceImageInformation);

  xmlNodePtr nodeNameFilePdf =
      xmlNewNode(NULL, (const xmlChar *)TAG_PDFFILENAME);
  ...
  //	dumpFragment((Unicode*)fileNamePDF, fileNamePDF->getLength(), uMap,
  // title);
  xmlNodeSetContent(
      nodeNameFilePdf,
      (const xmlChar *)xmlEncodeEntitiesReentrant(
          nodeNameFilePdf->doc, (const xmlChar *)title->getCString()));
  xmlAddChild(nodeSourceImageInformation, nodeNameFilePdf);

  xmlNodePtr nodeOCRProcessing =
      xmlNewNode(NULL, (const xmlChar *)TAG_OCRPROCESSING);
  ...
  xmlNodePtr nodeOCRProcessingStep =
      xmlNewNode(NULL, (const xmlChar *)TAG_OCRPROCESSINGSTEP);
  nodeOCRProcessingStep->type = XML_ELEMENT_NODE;
  xmlAddChild(nodeOCRProcessing, nodeOCRProcessingStep);

  xmlNodePtr nodeProcessingDate =
      xmlNewNode(NULL, (const xmlChar *)TAG_PROCESSINGDATE);
  ...
  xmlNodeSetContent(nodeProcessingDate,
                    (const xmlChar *)xmlEncodeEntitiesReentrant(
                        nodeProcessingDate->doc, (const xmlChar *)tstamp));

  xmlNodePtr nodeProcessingSoftware =
      xmlNewNode(NULL, (const xmlChar *)TAG_PROCESSINGSOFTWARE);
  nodeProcessingSoftware->type = XML_ELEMENT_NODE;
  xmlAddChild(nodeOCRProcessingStep, nodeProcessingSoftware);

  xmlNodePtr nodeSoftwareCreator =
      xmlNewNode(NULL, (const xmlChar *)TAG_SOFTWARE_CREATOR);
  nodeSoftwareCreator->type = XML_ELEMENT_NODE;
  xmlNodeSetContent(
      nodeSoftwareCreator,
      (const xmlChar *)xmlEncodeEntitiesReentrant(
          nodeSoftwareCreator->doc, (const xmlChar *)PDFALTO_CREATOR));
  xmlAddChild(nodeProcessingSoftware, nodeSoftwareCreator);

  xmlNodePtr nodeSoftwareName =
      xmlNewNode(NULL, (const xmlChar *)TAG_SOFTWARE_NAME);
  nodeSoftwareName->type = XML_ELEMENT_NODE;
  xmlNodeSetContent(nodeSoftwareName,
                    (const xmlChar *)xmlEncodeEntitiesReentrant(
                        nodeSoftwareName->doc, (const xmlChar *)PDFALTO_NAME));
  ...
  xmlNodePtr nodeSoftwareVersion =
      xmlNewNode(NULL, (const xmlChar *)TAG_SOFTWARE_VERSION);
  nodeSoftwareVersion->type = XML_ELEMENT_NODE;
  xmlNodeSetContent(
      nodeSoftwareVersion,
      (const xmlChar *)xmlEncodeEntitiesReentrant(
          nodeSoftwareName->doc, (const xmlChar *)PDFALTO_VERSION));
  ...
  //
  text = new TextPage(verbose, catalog, nodeLayout, imgDirName, baseFileName,
                      nsURI);
}
...
void XmlAltoOutputDev::addMetadataInfo(PDFDocXrce *pdfdocxrce) {
  ...
  // xmlNodePtr nodeSourceImageInfo = findNodeByName(docroot, (const
  // xmlChar*)TAG_SOURCE_IMAGE_INFO);
  ...
  xmlNodePtr creationDateNode =
      xmlNewNode(NULL, (const xmlChar *)"CREATIONDATE");
  xmlNodePtr modDateNode =
      xmlNewNode(NULL, (const xmlChar *)"MODIFICATIONDATE");
  ...
  //	xmlNewProp(nodeVersion, (const xmlChar*)ATTR_VALUE,(const
  // xmlChar*)PDFTOXML_VERSION);
  //
  //	infoNode =
  ...
  if (info.isDict()) {
    ...
    xmlNodeSetContent(
        titleNode, (const xmlChar *)xmlEncodeEntitiesReentrant(
    ...
    xmlNodeSetContent(subjectNode, (const xmlChar *)xmlEncodeEntitiesReentrant(
                                       subjectNode->doc,
                                       (const xmlChar *)content->getCString()));
    delete content;
...
void XmlAltoOutputDev::addStyles() {
  ...
  xmlNodePtr nodeSourceImageInfo =
      findNodeByName(docroot, (const xmlChar *)TAG_STYLES);
  for (int j = 0; j < getText()->fontStyles.size(); ++j) {
    ...
    // warning if the font name is very long, this can lead to buffer overflow,
    // so we truncate by default everything over 100 char
    strncpy(tmp, fontStyleInfo->getFontNameCS()->getCString(), sizeof(tmp) - 1);
    tmp[sizeof(tmp) - 1] = '\0';
    xmlNewProp(textStyleNode, (const xmlChar *)ATTR_FONTFAMILY,
               (const xmlChar *)tmp);
    delete fontStyleInfo->getFontNameCS();

    snprintf(tmp, sizeof(tmp), "%.3f", fontStyleInfo->getFontSize());
    xmlNewProp(textStyleNode, (const xmlChar *)ATTR_FONTSIZE,
               (const xmlChar *)tmp);

    snprintf(tmp, sizeof(tmp), "%s",
             fontStyleInfo->getFontType() ? "serif" : "sans-serif");
    xmlNewProp(textStyleNode, (const xmlChar *)ATTR_FONTTYPE,
               (const xmlChar *)tmp);

    snprintf(tmp, sizeof(tmp), "%s",
             fontStyleInfo->getFontWidth() ? "fixed" : "proportional");
    xmlNewProp(textStyleNode, (const xmlChar *)ATTR_FONTWIDTH,
               (const xmlChar *)tmp);

    snprintf(tmp, sizeof(tmp), "%s",
             fontStyleInfo->getFontColor()->getCString());
    xmlNewProp(textStyleNode, (const xmlChar *)ATTR_FONTCOLOR,
               (const xmlChar *)(tmp + 1));
    ...
    if (strcmp(tmp, ""))
      xmlNewProp(textStyleNode, (const xmlChar *)ATTR_FONTSTYLE,
                 (const xmlChar *)tmp);
...
xmlNodePtr XmlAltoOutputDev::findNodeByName(xmlNodePtr rootnode,
...
GString *XmlAltoOutputDev::getInfoDate(Dict *infoDict, const char *key) {
  ...
  if (infoDict->lookup(key, &obj)->isString()) {
    ...
    if ((n = sscanf(s, "%4d%2d%2d%2d%2d%2d", &year, &mon, &day, &hour, &min,
                    &sec)) >= 1) {
      switch (n) {
      case 1:
        mon = 1;
...
static SplashStrokeAdjustMode mapStrokeAdjustMode[3] = {
    splashStrokeAdjustOff, splashStrokeAdjustNormal, splashStrokeAdjustCAD};
...
SplashFont *XmlAltoOutputDev::getSplashFont(GfxState *state,
                                            SplashCoord *matrix) {
  SplashFont *font;
  GfxFont *gfxFont;
  GfxFontLoc *fontLoc;
...
#if LOAD_FONTS_FROM_MEM
      if ((ffT1C =
               FoFiType1C::make(fontBuf->getCString(), fontBuf->getLength()))) {
#else
...
void XmlAltoOutputDev::drawChar(GfxState *state, double x, double y, double dx,
  ...
  if (parameters->getOcr() == gTrue) {
    ...
    if ((uLen == 0 ||
         ...
         (uLen > 1 &&
          (globalParams->getTextEncodingName()->cmp(ENCODING_UTF8) == 0) &&
          !isUTF8(u, uLen)))) {
      // when len is gt 1 check if sequence is valid, if not replace by
      // placeholder
      //&& globalParams->getApplyOCR())
      //  as a first iteration for dictionnaries, placing a placeholder, which
      //  means creating a map based on the font-code mapping to unicode from :
      //  https://unicode.org/charts/PDF/U2B00.pdf
      GString *fontName = new GString();
      if (state->getFont()
              ->getName()) { // AA : Here if fontName is NULL is problematic
        fontName = state->getFont()->getName()->copy();
        fontName = fontName->lowerCase();
      }
      GString *fontName_charcode = fontName->append(
          to_string(c).c_str()); // for performance and simplicity only
                                 // appending is done
  ...
  text->addChar(state, x, y, dx, dy, c, nBytes, u, uLen, splashFont,
                isNonUnicodeGlyph);
}

void XmlAltoOutputDev::updateCTM(GfxState *state, double m11, double m12,
                                 double m21, double m22, double m31,
                                 double m32) {
  for (int i = 0; i < 6; ++i) {
    curstate[i] = state->getCTM()[i];
  }
...
T3FontCache::T3FontCache(Ref *fontIDA, double m11A, double m12A, double m21A,
                         ...
                         int glyphHA, GBool validBBoxA, GBool aa) {
  int i;
...
void XmlAltoOutputDev::eoClip(GfxState *state) { text->eoClip(state); }

void XmlAltoOutputDev::clipToStrokePath(GfxState *state) {
  text->clipToStrokePath(state);
}

void XmlAltoOutputDev::doPath(GfxPath *path, GfxState *state,
                              GString *gattributes) {
  // if (parameters->getDisplayImage())
  {
...
void XmlAltoOutputDev::saveState(GfxState *state) { text->saveState(state); }
...
void XmlAltoOutputDev::colortoString(char *buf, size_t buflen,
                                     GfxRGB rgb) const {
...
void XmlAltoOutputDev::drawSoftMaskedImage(
...
void XmlAltoOutputDev::drawImage(GfxState *state, Object *ref, Stream *str,
  ...
  // if (parameters->getDisplayImage())
  {
    ...
    if ((ref != NULL) && (ref->isRef())) {
      ...
      for (int i = 0; i < lPictureReferences->getLength(); i++) {
        PictureReference *pic_reference =
            (PictureReference *)lPictureReferences->get(i);
    ...
    // if (index == -1)
    {
      ...
      if (height > 8 && width > 8 &&
          imageIndex < parameters->getFilesCountLimit()) {
        imageIndex += 1;
        // Save this in the references
        //			text->drawImage(state, ref, str, width, height,
        // colorMap, maskColors,inlineImg, dumpJPEG, imageIndex);
        ext = text->drawImageOrMask(state, ref, str, width, height, colorMap,
                                    maskColors, parameters->getDisplayImage(),
                                    false,
                                    imageIndex); // not a mask
        lPictureReferences->append(
            new PictureReference(reference, flip, imageIndex, ext));
      }

    } /*else {
        //HD : in order to avoid millions of small (pixel) images
        if (height > 8 && width > 8 && imageIndex <
    parameters->getFilesCountLimit()) { imageIndex += 1;
            //			text->drawImage(state, ref, str, width, height,
    colorMap, maskColors,inlineImg, dumpJPEG, imageIndex); ext =
    text->drawImageOrMask(state, ref, str, width, height, colorMap, maskColors,
    inlineImg, false, imageIndex); // not a mask lPictureReferences->append(new
    PictureReference(reference, flip, imageIndex, ext));
        }
    }*/
  }
}
...
void XmlAltoOutputDev::drawImageMask(GfxState *state, Object *ref, Stream *str,
  ...
  if ((ref != NULL) && (ref->isRef())) {
    ...
    for (int i = 0; i < lPictureReferences->getLength(); i++) {
      ...
      if ((pic_reference->reference_number == reference) &&
        ...
        // We already created a file for this picture
        index = pic_reference->picture_number;
        //				printf("MASK IMAGE ALREADY SEEN
        // flip=%d\t%d %d\n",flip,reference,index);
        break;
      }
    }
  ...
  // if (index == -1)
  {
    // if (parameters->getDisplayImage())
    {
      ...
      if (height > 8 && width > 8 &&
        ...
        imageIndex += 1;
        // Save this in the references
        ext = text->drawImageOrMask(state, ref, str, width, height, NULL, NULL,
                                    parameters->getDisplayImage(), true,
                                    imageIndex); // mask
        //                ext= text->drawImageMask(state, ref, str, width,
  ...
  parameters->getFilesCountLimit()) { imageIndex += 1;
              ...
              ext = text->drawImageOrMask(state, ref, str, width, height, NULL,
  NULL, inlineImg, true, imageIndex); // mask
              //			text->drawImageMask(state, ref, str,
  width, height, invert, inlineImg,
              //				dumpJPEG, imageIndex);
              lPictureReferences->append(new PictureReference(reference, flip,
  imageIndex, ext));
          }
      }
  }*/
...
void XmlAltoOutputDev::generateOutline(GList *itemsA, PDFDoc *docA,
                                       int levelA) {
  UnicodeMap *uMap;
  GString *enc;
  idItemToc = 0;
...
GBool XmlAltoOutputDev::dumpOutline(xmlNodePtr parentNode, GList *itemsA,
                                    PDFDoc *docA, UnicodeMap *uMapA, int levelA,
                                    int idItemTocParentA) {
  // store them in a list
  ...
  for (i = 0; i < itemsA->getLength(); ++i) {
    ...
    dumpFragment(((OutlineItem *)itemsA->get(i))->getTitle(),
                 ((OutlineItem *)itemsA->get(i))->getTitleLength(), uMapA,
                 title);
    ...
    if (((OutlineItem *)itemsA->get(i))->getAction()) {
      ...
      case actionGoTo:
        ...
        if ((dest = ((LinkGoTo *)((OutlineItem *)itemsA->get(i))->getAction())
                        ->getDest())) {
          dest = dest->copy();
        } else if ((namedDest = ((LinkGoTo *)((OutlineItem *)itemsA->get(i))
                                     ->getAction())
                                    ->getNamedDest())) {
        ...
        // LAUNCH action
      case actionLaunch:
        // fileName = ((LinkLaunch *) ((OutlineItem *)
        // itemsA->get(i))->getAction())->getFileName(); delete fileName;
        break;
    ...
    //   	  	xmlNodeSetContent(nodeString,(const
    //   xmlChar*)xmlEncodeEntitiesReentrant(nodeString->doc,(const
    //   xmlChar*)title->getCString()));
    xmlNodeSetContent(
        nodeString, (const xmlChar *)xmlEncodeEntitiesReentrant(
                        nodeString->doc, (const xmlChar *)title->getCString()));
    ...
    if (((OutlineItem *)itemsA->get(i))->hasKids()) {
      dumpOutline(nodeItem, ((OutlineItem *)itemsA->get(i))->getKids(), docA,
                  uMapA, levelA + 1, idItemCurrent);
    }
