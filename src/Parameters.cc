...
void Parameters::setSkipGraphs(GBool skip) {
  lockGlobalParams;
  skipGraphs = skip;
  unlockGlobalParams;
}
...
void Parameters::saveToXML(const char *fileName, int firstPage, int lastPage) {
  ...
  xmlNewProp(param, (const xmlChar *)"help",
             (const xmlChar *)"first page to convert");
  ...
  xmlNewProp(param, (const xmlChar *)"type", (const xmlChar *)"int");
  xmlNewProp(param, (const xmlChar *)"help",
             (const xmlChar *)"last page to convert");
  if (lastPage == 0) {
    lastPage = 10000;
  }
  ...
  xmlNewProp(param, (const xmlChar *)"help",
             (const xmlChar *)"do not extract text");
  if (!getDisplayText()) {
  ...
  xmlNewProp(param, (const xmlChar *)"default", (const xmlChar *)"False");
  xmlNewProp(param, (const xmlChar *)"help",
             (const xmlChar *)"do not extract images");
  if (!getDisplayImage()) {
    snprintf(tmp, sizeof(tmp), "True");
  } else {
    snprintf(tmp, sizeof(tmp), "False");
