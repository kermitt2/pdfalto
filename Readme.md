...
Usage: pdfalto [options] <PDF-file> [<xml-file>]
  ...
  -onlyGraphsCoord               : do not extract image files (coordinates only in ALTO)
  -skipGraphs                    : completely skip graphics/image processing
...
* `.xml_data/` subdirectory containing the vectorial (.vec) and bitmap images (.png) embedded in the PDF, this is generated by default - when the option `-onlyGraphsCoord` or `-skipGraphs` is not present. This extraction slows down the process very significantly, so if no image is required, use the option `-onlyGraphsCoord` (for coordinates only) or `-skipGraphs` (to completely skip graphics processing). When the images are not extracted with `-onlyGraphsCoord`, image elements with layout properties still appear in the ALTO file, but they reference no extracted image files. With `-skipGraphs`, graphics processing is completely skipped.
